...
class Date extends React.Component {
	...
	handleChangeDate(value) {
		...
		let { curData, curComp } = editConfig 
		let { parentComp } = curData
		data.feature.format_date = value
		actions['updateComp'](null, parentComp? parentComp: data)
	}
	handleChangeTime(value) {
		...
		let { curData, curComp } = editConfig 
		let { parentComp } = curData
		data.feature.format_time = value
		actions['updateComp'](null, parentComp? parentComp: data)
	}
	render() {
		const options = [
		...
		const options_date = [
			{ value: '-',      show: 'YYYY-MM-DD', type: 1 },
			{ value: '/',      show: 'YYYY/MM/DD', type: 1 },
			{ value: 'rongyi', show:'YYYY年M月D日', type: 2 }
		]
		const options_time = [
			{ value: ':',      show: 'HH:mm:ss', type: 1 },
			{ value: 'rongyi', show: 'HH:mm',    type: 2 }
		]
		return (
				...
				<Collapse activeKey={['0']}>
					<Panel header='模式选择' key={0}>
						<div key={0} className="pgs-row">
							...
							<Select defaultValue={options[0].show} style={{ width: 120 }} onChange={_ => this.handleChange(_)}>
     							{     
						...
						<div key={1} className="pgs-row">
							...
							<Select defaultValue={options_date[0].show} style={{ width: 120 }} onChange={_ => this.handleChangeDate(_)}>
     							{     
						...
						<div key={2} className="pgs-row">
							...
							<Select defaultValue={options_time[0].show} style={{ width: 120 }} onChange={_ => this.handleChangeTime(_)}>
     							{     
